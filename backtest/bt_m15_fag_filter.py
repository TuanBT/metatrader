#!/usr/bin/env python3
"""
M15 Impulse FAG Entry EA v2.0 — Filter Sensitivity Tests
Tests M15 and M5 with various MTF MinAgree settings.
Goal: find best filter that balances trade count vs quality.
"""
...
DEPOSIT    = 500
LEVERAGE   = 100
RESULT_MD  = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bt_m15_fag_filter_results.md")
...
# (symbol, tf, from_date, to_date, UseMTF, MinAgree, label)
TESTS = [
    # ── M15: no filter ────────────────────────────────────────────
    ("XAUUSDm", "M15", "2025.01.01", "2026.02.01", "false", "3", "M15 NoFilter 2025"),
    ("XAUUSDm", "M15", "2024.01.01", "2025.01.01", "false", "3", "M15 NoFilter 2024"),
    # ── M15: MinAgree=2 ───────────────────────────────────────────
    ("XAUUSDm", "M15", "2025.01.01", "2026.02.01", "true",  "2", "M15 MinAg=2  2025"),
    ("XAUUSDm", "M15", "2024.01.01", "2025.01.01", "true",  "2", "M15 MinAg=2  2024"),
    # ── M5: MinAgree=2 ────────────────────────────────────────────
    ("XAUUSDm", "M5",  "2025.01.01", "2026.02.01", "true",  "2", "M5  MinAg=2  2025"),
    ("XAUUSDm", "M5",  "2024.01.01", "2025.01.01", "true",  "2", "M5  MinAg=2  2024"),
    # ── M5: no filter ─────────────────────────────────────────────
    ("XAUUSDm", "M5",  "2025.01.01", "2026.02.01", "false", "3", "M5  NoFilter 2025"),
    ("XAUUSDm", "M5",  "2024.01.01", "2025.01.01", "false", "3", "M5  NoFilter 2024"),
]
...
def main():
    ...
    for (symbol, tf, from_d, to_d, use_mtf, min_agree, label) in TESTS:
        print(f"\n{'='*60}")
        print(f"  {label}")
        print(f"{'='*60}")
        ...
        cfg = dict(BASE)
        cfg["InpUseMTFConsensus"] = use_mtf
        cfg["InpMTFMinAgree"]     = min_agree

        print(f"  MTF={use_mtf} MinAg={min_agree} | {symbol} {tf} {from_d}–{to_d}")
    ...
    md = [f"# M15 FAG v2.0 — Filter Sensitivity", f"*Generated: {ts}*",
          f"**Deposit** $500 | **Risk** $10/trade | Trail 0.5R→0.25R | No TP\n",
          "| Label | P&L% | Deals | SL | WR% |",
          "|---|---|---|---|---|"]
    for r in results:
        if "error" in r:
            ...
            md.append(f"| {r['label']} | {r['profit_pct']:+.2f}% | {r['deals']} | {r['sl']} | {wr}% |")
