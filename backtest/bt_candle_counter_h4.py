#!/usr/bin/env python3
"""
Candle Counter EA v1.2 — H4 Multi-Year Validation
Tests H4 NoFilter + H4 EMA50 across 4 years: 2022, 2023, 2024, 2025
"""
...
DEPOSIT    = 500
LEVERAGE   = 100
RESULT_MD  = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bt_candle_counter_h4_results.md")
...
# Test matrix: H4 NoFilter + H4 EMA50 across 4 years
CONFIGS = [
    ("H4 NoFilter", "false", "50"),
    ("H4 EMA50",    "true",  "50"),
    ("H4 EMA200",   "true",  "200"),
]
...
TESTS = []
for (cfg_name, ema_on, ema_period) in CONFIGS:
    for (yr_from, yr_to, yr) in YEARS:
        TESTS.append((cfg_name, ema_on, ema_period, yr_from, yr_to, yr))


def main():
    results = []

    for (cfg_name, ema_on, ema_period, yr_from, yr_to, yr) in TESTS:
        label = f"{cfg_name} {yr}"
        print(f"\n{'='*62}")
        print(f"  {label}")
        print(f"{'='*62}")
    ...
    rows = ["# Candle Counter v1.2 — H4 Multi-Year Validation",
            f"*Generated: {ts}*",
            "**XAUUSDm H4 | Lot=0.01 | No time/ATR filter**\n",
    ...
    for cfg_name, _ in CONFIGS:
        ...
        for yr in ["2022", "2023", "2024", "2025"]:
            ...
            if d and "error" not in d:
                mark = "+" if d["profit_pct"] > 0 else "-"
                cells.append(f"**{d['profit_pct']:+.1f}%** ({d['deals']}d)" if d["profit_pct"] > 0
                             else f"{d['profit_pct']:+.1f}% ({d['deals']}d)")
                if d["profit_pct"] <= 0:
                    all_pos = False
            else:
                cells.append("—")
                all_pos = False
        rows.append(f"| {cfg_name} | {' | '.join(cells)} | {'✅' if all_pos else '⚠'} |")
