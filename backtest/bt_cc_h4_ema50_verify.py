#!/usr/bin/env python3
"""Re-run H4 EMA50 across all 4 years cleanly."""
...
def run_bt(tf, yr_from, yr_to, inputs):
    ...
    lines = [
        ...
        f'echo Symbol=XAUUSDm >> "{ini}"',
        f'echo Period={tf} >> "{ini}"',
        f'echo Model=1 >> "{ini}"',
        f'echo Optimization=0 >> "{ini}"',
        f'echo FromDate={yr_from} >> "{ini}"',
        f'echo ToDate={yr_to} >> "{ini}"',
...
tests = [
    ("H4 EMA50 2025", "H4", "2025.01.01", "2026.02.01"),
    ("H4 EMA50 2024", "H4", "2024.01.01", "2025.01.01"),
    ("H4 EMA50 2023", "H4", "2023.01.01", "2024.01.01"),
    ("H4 EMA50 2022", "H4", "2022.01.01", "2023.01.01"),
]

for (label, tf, yr_from, yr_to) in tests:
    print(f"\n[{label}]")
    kill_mt5()
    date_str = get_date()
    cfg = {
